...
def SubCalc(A,b): 
    ...
    for i in range(0,n-1,b): # Column index, stepsize b
        A[i:n,i:i+b] = Factorize(A[i:n,i:i+b]) # A=LU of n-i x b matrix below main diagonal
        L = np.tril(A[i:i+b,i:i+b]); np.fill_diagonal(L, 1) # L part of A=LU
        ''' Solving of equation system. BLAS rountine blas.dtrsm not working on HARRIS therefore the inverse was used 
        as a compromise. If available change np.dot(lin.inv,...) to blas.dtrms(1,L,A[i:i+b,i+b:n],lower=1) '''
        A[i:i+b,i+b:n] = np.dot(lin.inv(L),A[i:i+b,i+b:n]) 
        A[i+b:n,i+b:n] = A[i+b:n,i+b:n] - np.dot(A[i+b:n,i:i+b], A[i:i+b,i+b:n]) # update of active submatrix
    return A

def LU_Block(dim,b): # dimension of nxn-matrix, block size
    """ Blocked LU-Decomp without pivoting for non-singular matrix A """

    A = np.random.uniform(90,100,(dim,dim)) # Values taken from random uniform distribution
    B = np.copy(A) # deep copy to not overwrite A
    
    start = datetime.datetime.now() # Starts the timer for the main calculation
    B = SubCalc(B,b) # Actually computes the decomp
